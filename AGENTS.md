...
- `explorer_zen.py` — entire program. All prompts, dashboard text, and log strings are in Russian. Constants to tune live near the top: `AI_MODEL`, `REASONING_EFFORT`, `LOOP_INTERVAL`, `API_TIMEOUT`, `WIKI_SEARCH_TIMEOUT`, `WIKI_SUMMARY_TIMEOUT`, `MAX_RETRIES`, `BASE_DELAY`, `MAX_SESSIONS` (None = infinite), `MAX_WORLD_PICTURE_ENTRIES`, `MAX_LONG_TERM_KNOWLEDGE_ENTRIES`, `BAR_WIDTH`, `MAX_RECENT_QUERIES`, `WIKI_EXTRACT_MAX_CHARS`, `MEMORY_FILE`, `REPORTS_DIR`. Indent constants near top: `_DASH_FIELD_MARGIN=22`, `_DASH_MIN_FIELD=20`, `_DASH_GOAL_INDENT=14`, `_DASH_DETAIL_INDENT=5`, `_DASH_RIGHT_PAD=2`. Color helpers: `_Color` class (instances `_RESET`, `_DIM`, `_BOLD`, `_CYAN`, `_GREEN`, `_YELLOW`, `_RED`, `_MAGENTA`, `_GREY` — all callable, return SGR). Helper functions: `parse_llm_response`, `update_world_picture`, `write_session_report`, `synthesize_world_picture`, `write_synthesis_report`, `_world_picture_cap_reached`, `invalidate_memory_cache`, `save_memory` (writes `memory.json` and invalidates memory cache), `_default_memory` / `_repair_memory_if_corrupt` / `_migrate_memory` (used by `init_system`), `_set_service_state` (unified helper for the two service-state dicts), `_bump_memory_counter` (increments a counter field with default 0), `_mark_failure` (sets `_LAST_SESSION_OK=False` and renders dashboard; caller is responsible for `_bump_memory_counter` and f-string formatting so the new counter value is visible), `build_session_prompt` / `build_synthesis_prompt` (LLM prompt builders), `_maybe_rotate_next_query` / `_maybe_run_synthesis` (inline blocks extracted from `execute_session` for readability). Dashboard renderers: `_render_bar(count, cap, color)` (module-level, used by `_world_picture_lines`), `_header_line` / `_status_line` / `_article_lines` / `_world_picture_lines` / `_services_line` / `_footer_lines` (6 sub-helpers of `_build_dashboard_lines`). Wiki pipeline: `search_wikipedia` → `prop=revisions&rvprop=content&rvslots=main` → `_wikitext_to_plain` (27-line stripper: templates, refs, tables, links, headings, etc.) → truncate to `WIKI_EXTRACT_MAX_CHARS=5000`. Quality helpers: `_extract_quote`, `_verify_grounded` (case-insensitive normalize), `_grounding_filter` (drops fabricated quotes, keeps no-quote by default = "soft limit"), `_is_duplicate` (difflib.SequenceMatcher, ratio>0.85, last 30 entries). Read `execute_session` before refactoring — its body is now ~60 lines and reads as a linear sequence.
...
The OpenRouter chat-completions payload includes `reasoning: {effort: REASONING_EFFORT}` (see [OpenRouter docs](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens)). Allowed values for DeepSeek V4 Flash: `xhigh`, `high`, `medium`, `low`, `minimal`, `none`. `xhigh` maps to max reasoning (~95% of `max_tokens`); `high` ≈ 80%. The chain-of-thought arrives in `choices[0].message.reasoning`; the parser reads only `content`, so reasoning tokens are silently dropped. Side effect: when thinking is on, `temperature`/`top_p` are accepted by the API but ignored by DeepSeek (by design, not a bug).
