...
REPORTS_DIR = "reports"
DEFAULT_NEXT_QUERY = "Теория всего"  # стартовая тема для нового memory.json
STATUS_CYAN, STATUS_YELLOW, STATUS_RED, STATUS_GREEN, STATUS_GREY = 36, 33, 31, 32, 90
...
def _format_bullet_list(items, empty_msg):
    """Возвращает многострочный bullet-list для Markdown, или empty_msg, если items пуст."""
    if items:
        return chr(10).join(f"* {x}" for x in items)
    return empty_msg
...
_STATUS_COLOR = {
    "ПРОБУЖДЕНИЕ": STATUS_CYAN,
    "ПОИСК ДАННЫХ": STATUS_CYAN,
    "ИНФЕРЕНС ИИ": STATUS_CYAN,
    "СЕТЕВАЯ ПАУЗА": STATUS_YELLOW,
    "СБОЙ КАНАЛА ДАННЫХ": STATUS_RED,
    "АНАЛИЗ СТРУКТУРЫ": STATUS_CYAN,
    "ПАРСИНГ ОТВЕТА": STATUS_CYAN,
    "КОМПИЛЯЦИЯ ОТЧЕТА": STATUS_CYAN,
    "СЕССИЯ УСПЕШНО СИНХРОНИЗИРОВАНА": STATUS_GREEN,
    "КРИТИЧЕСКИЙ СБОЙ СЕТИ": STATUS_RED,
    "РЕЖИМ ОЖИДАНИЯ (СОН)": STATUS_GREY,
    "ОСТАНОВКА ПО ЗАПРОСУ": STATUS_YELLOW,
    "ЗАВЕРШЕНИЕ": STATUS_GREEN,
}
...
def _render_bar(count, cap_val, color):
    """Прогресс-бар: заполненные/пустые клетки в цветовой обёртке. color — _Color инстанс."""
    cap_safe = cap_val if cap_val > 0 else 1
    ratio = max(0.0, min(1.0, count / cap_safe))
    filled = int(round(ratio * BAR_WIDTH))
    bar_str = "█" * filled + "░" * (BAR_WIDTH - filled)
    return color() + bar_str + _RESET()
...
def _world_picture_lines(mem):
    ...
    bar_laws = _render_bar(laws_count, cap, _GREEN)
    bar_px = _render_bar(paradoxes_count, cap, _MAGENTA)
    bar_lnk = _render_bar(links_count, cap, _CYAN)
    return [
...
def write_session_report(discovery, memory, parsed):
    ...
    report_content = (
        ...
        f"{_format_bullet_list(new_p, 'Существующие константы не изменились.')}\n\n"
        f"### Обнаруженные парадоксы:\n"
        f"{_format_bullet_list(new_px, 'Новых противоречий не выявлено.')}\n\n"
        f"### Семантические связи:\n"
        f"{_format_bullet_list(new_l, 'Новых ассоциативных мостов не построено.')}\n\n"
...
def execute_session():
    ...
    target_query = memory.get("next_query", DEFAULT_NEXT_QUERY)
